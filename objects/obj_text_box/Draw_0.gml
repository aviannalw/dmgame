...
//loop through the pages
		for(var p =0; p < page_number; p++) //if the length of the page number is less than p, add 1
			...
			for (var c = 0; c < text_length[p]; c++)
				...
				{
				//if the current looping character is ater a line break
...
if draw_char == text_length[page] && page == page_number -1 //if we're on the final page...
{
		...
		//draw the options
		var _op_space = 15; //i'll have to mess around with these values, vertical distance between options
		var _op_bord = -4; // text space from sides of box
...
for (var c = 0; c < draw_char; c++)
{
	...
	draw_text(char_x[c, page] + 54, char_y[c, page], char[c, page]); //i had to manually offset the x value because it was typing on the left side of the screen
}